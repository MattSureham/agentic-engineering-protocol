...
- **Probed interface only:** Launches pass exactly the probed flags the launcher constructs: `-p`, `--output-format json`, `--tools`, `--max-budget-usd`, and `--allowedTools` (omitted when the profile's `allowed_tools` is empty). `--resume` session re-entry is probe-verified capability but the current launcher does not pass it. The verified live profile is `tools` and `allowed_tools` both `Read,Edit,Write,Bash` per [`EVIDENCE-20260817T023721Z-live-profile-probe`](EVIDENCE/EVIDENCE-20260817T023721Z-live-profile-probe.md); the originally probed `tools ""` profile remains valid with an empty `allowed_tools`. Headless permission denial is machine-readable: a success-shaped envelope with a non-empty `permission_denials` array and absent work product. Any launch configuration beyond a probed profile requires new recorded probe evidence before reliance; unrecognized envelope shapes fail closed as `session_error`.

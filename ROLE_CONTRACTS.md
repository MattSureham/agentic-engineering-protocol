...
The dispatcher emits the next-role decision; starting a session that performs the role is host-specific invocation and is **not** repository-native. In v1 the adapter is a documented manual step: the operator (or the current session, at a role boundary) starts the next participant session and supplies the emitted role, eligibility constraints, role-contract reference, and expected records and commands. No repository code launches, simulates, or pretends to launch a participant. A host that exposes a genuine durable programmatic launch interface may implement a real adapter in a future owner-approved milestone without changing the repository-native dispatcher.

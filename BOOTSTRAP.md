...
that may outlive the current turn — must not be "started then forgotten":
its lifecycle state must remain queryable without access to the originating
session, and its terminal state must be recorded whether the task succeeded,
failed, or was cancelled.

HANDOFF.md fixes the concrete lifecycle schema and carries the live
background-task state. A participant taking over must first reconcile that
state: any task still recorded as running whose process or remote reference
is dead must be marked orphaned rather than treated as alive.

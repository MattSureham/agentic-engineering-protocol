...
- **Generated UTC:** `2026-08-14T09:36:00Z`
- **Prepared by:** `ClaudeCode/root`
- **Period covered:** Owner direction toward automated participant rotation, live host-capability probing, and evidence-backed specification evolution authorizing `MILESTONE-20260814T092504Z-host-rotation-v1`, from the dispatcher's terminal wait state at `2026-08-14T09:00:30Z` through the authority recording of `2026-08-14T09:36:00Z`
- **Specification status reviewed:** Root `PROJECT_SPEC.md` is `ACCEPTED`, now including the owner-approved Host adapter and participant rotation phase alongside the Authorized milestone pipeline and Automated role dispatch phases
- **Implementation/reference state:** The pipeline and dispatch milestones remain `ACCEPTED` with owning issues `CLOSED`. The rotation milestone is `AUTHORIZED` at attempt 0; no implementation exists yet. Live probes verified the host launch interface (Claude Code CLI `2.1.118` headless sessions with JSON envelopes, budget caps, machine-readable budget-exhaustion failure, resumable sessions) and confirmed no `paseo` binary or project exists. The dispatcher selects milestone 3 and routes to the implementer role.
- **Prior checkpoint:** `2026-08-14T09:00:30Z` by `ClaudeCode/dispatch-record` (superseded by this authority record)
...
| Authorize the host adapter and participant rotation milestone | Close the rotation loop now that live probes verify a genuine host launch interface, while keeping adapters subordinate to dispatch decisions and participant failures distinct from authority gaps | Accepted rotation phase (`ROTATE-001`–`ROTATE-008`) and milestone-3 contract; accepted ADR pinning the interface to probe evidence; participant registry, append-only ledger, bounded retry/rotation, stub-only tests; no implementation yet | Owner direction `2026-08-14`; [`EVIDENCE-20260814T092504Z-host-capability-probe`](EVIDENCE/EVIDENCE-20260814T092504Z-host-capability-probe.md); [`ADR-20260814T092504Z-host-adapter-rotation`](ADR/ADR-20260814T092504Z-host-adapter-rotation.md); [`ISSUE-20260814T092504Z-host-adapter-rotation`](ISSUES/ISSUE-20260814T092504Z-host-adapter-rotation.md); pipeline status selects milestone 3; dispatcher routes to implementer |
...
| Host adapter, participant registry, and rotation ledger (authorized, not yet implemented) | Execute dispatcher decisions through the verified host interface without routine human routing | Accepted rotation phase/ADR and live probe evidence; implementation, stub-launcher tests, and independent review pending under milestone 3 | Real quota spend by tooling; host rate limits, concurrency, and envelope stability across versions remain unprobed |
...
- **What is directly verified:** Prior structural/pipeline/dispatch targets and reviews remain valid; milestone digests 1/2 unchanged and milestone-3 digest `a38bb7bfd1511045e8e09b4a0dc6af7893f24a8a833e9a3faa444660cc3b977b` computed by the accepted pipeline parser; pipeline status selects milestone 3 (`AUTHORIZED`); the dispatcher routes to `implementer` with the exact READY transition command; the structural validator passes; the full suite passes 63 tests; the three host probes are captured with exact commands, envelopes, and exit codes.
...
- **What was not run or remains unverified:** The rotation milestone has no implementation yet; host rate limits, concurrent sessions, long-running stability, authentication-mode variation, and CLI envelope stability across versions are unprobed. Dedicated Markdown linting is unavailable; broader platform portability, CommonMark conformance, concurrency guarantees, authenticated identity, and large-scale coordination remain unverified.
- **Known regressions or unresolved risks:** None blocking. Four deferred capability areas remain `BLOCKED`.

## Human attention required

No owner decision is currently pending. On `2026-08-14` the owner directed automated participant rotation with evidence-first capability bounding; the capability boundary is now probed and recorded, and the Host adapter and participant rotation phase, compatible accepted ADR, and owning issue are recorded through specification evolution. The milestone is `AUTHORIZED`; an implementer may begin without another routine owner prompt. New product scope beyond the accepted milestones, package runtime distribution, unprobed host APIs, human-blocker resolution, concurrent-writer guarantees, authenticated identity, large-scale coordination, or tracker integration still require new owner authority.
...
- **Expected owner action before then:** `NONE`; the rotation milestone is authorized and the dispatcher routes the next participant

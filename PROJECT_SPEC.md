...
- **PIPELINE-003 — Lifecycle:** The supported milestone states are `AUTHORIZED`, `READY`, `IN_PROGRESS`, `AWAITING_PEER_REVIEW`, `CHANGES_REQUIRED`, `ACCEPTED`, and `BLOCKED_HUMAN_AUTHORITY`. Only the transitions defined by the compatible accepted ADR are permitted. `BLOCKED_HUMAN_AUTHORITY` returns to `IN_PROGRESS` only under the blocked-exit conditions of PIPELINE-009.
...
- **PIPELINE-009 — Blocked-state exit semantics:** Every milestone state the pipeline may enter MUST have a defined, verifiable exit; no machine state may be enterable yet impossible to leave through a legal transition. `BLOCKED_HUMAN_AUTHORITY` returns to `IN_PROGRESS` only when the durable blocker issue records the human technical owner's decision satisfying its unblock condition, the transition evidence cites that recorded decision, and no other unresolved human-authority blocker applies; the resume preserves the interrupted attempt. Exiting `BLOCKED_HUMAN_AUTHORITY` resolves only the authority blocker — it MUST NOT be read as authorizing any separately gated execution, resource budget, or external side effect, whose own conditions remain in force until independently satisfied. Quota, budget, or other participant/resource failures MUST NOT by themselves produce `BLOCKED_HUMAN_AUTHORITY` (reaffirming ROTATE-004).
...
| `2026-09-23T01:32:06Z` | Added PIPELINE-009 blocked-state exit semantics and amended PIPELINE-003 plus ADR-20260814T015817Z decision 5: every enterable state must have a verifiable exit; `BLOCKED_HUMAN_AUTHORITY → IN_PROGRESS` gated on the recorded owner decision in the entry blocker issue; unblock resolves only the authority gate and authorizes no separately gated execution or resource consumption; participant/resource failures alone never produce `BLOCKED_HUMAN_AUTHORITY` | The accepted contract defined entry into `BLOCKED_HUMAN_AUTHORITY` but no exit edge, discovered when the discovery milestone's human gate was satisfied yet no legal transition out existed; the 2026-09-22 entry record is preserved with an attributable ROTATE-004 correction (the true gate was the owner's resource directive, not quota exhaustion) | Human technical owner `MattSureham` (explicit approval with bounded scope: no new states, no state-machine redesign) | [ISSUE-20260923T013206Z-pipeline-blocked-exit](ISSUES/ISSUE-20260923T013206Z-pipeline-blocked-exit.md), [ISSUE-20260922T073608Z-codex-quota-authorization](ISSUES/ISSUE-20260922T073608Z-codex-quota-authorization.md), [ADR-20260814T015817Z](ADR/ADR-20260814T015817Z-authorized-milestone-pipeline.md) status history |

...
- **Resolution state:** Exact approved wording is implemented, coordinator-verified, and independently `APPROVED` at the immutable target. Keep the issue in `REVIEW` until the coordinator verifies this review-only boundary, closes the issue on a complete pass, and records any failure instead of relying on this disposition alone.
...
Verify that the independent-review commit changes only `HANDOFF.md`, preserves target protocol tree `8676b85d292676b7e198d10414961bf3657bf578`, and passes the critical identity, semantic, Markdown, and task-terminal checks; on a complete pass close `AEP-20260805T095724Z-specification-evolution` and prepare the reviewed chain for publication, otherwise record the failure and return the issue to the appropriate earlier state.
